...
document.addEventListener("DOMContentLoaded", function () {
  const productNamesMap = {
    "Product 1": "ProFlex Squash Racket",
    "Product 2": "SpeedCore Match Ball",
    "Product 3": "CourtGrip Squash Shoes",
    "Product 4": "ImpactGuard Eye Wear",
    "Product 5": "PowerShot Training Tool",
    "Product 6": "EliteGrip Replacement Set",
    "Product 7": "AeroLite Squash Racket",
    "Product 8": "ControlMax Wrist Band"
  };
...
// Slider auto-play with smooth animations
jQuery(document).ready(function() {
    // Auto-start carousel with custom interval
    var carousel = jQuery('#mainCarousel');
    if (carousel.length) {
        carousel.carousel({
            interval: 5000,
            pause: 'hover'
        ...
        // Add fade effect to carousel caption on slide change
        carousel.on('slide.bs.carousel', function() {
            jQuery('.slider-carousel-caption').fadeOut(300);
        });

        carousel.on('slid.bs.carousel', function() {
            jQuery('.slider-carousel-caption').fadeIn(600);
        });
    }
    ...
    jQuery(window).on('scroll', function() {
        jQuery('.fade-in-section').each(function() {
    ...
    jQuery('.pricing-section, .products-section, .blog-section').addClass('fade-in-section');

    // Product card hover effect - show "Add to Cart" button
    jQuery('.product-card, .category-card').hover(
        function() {
            jQuery(this).find('.btn').css('opacity', '1');
        },
        function() {
            jQuery(this).find('.btn').css('opacity', '0');
        }
    );
});
