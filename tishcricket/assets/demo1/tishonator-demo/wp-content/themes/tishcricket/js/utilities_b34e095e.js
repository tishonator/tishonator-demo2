...
document.addEventListener("DOMContentLoaded", function () {
  const productNamesMap = {
    "Product 1": "ProEdge English Willow Bat",
    "Product 2": "StrikeMaster Batting Pads",
    "Product 3": "EliteGrip Pro Gloves",
    "Product 4": "Champion Team Jersey",
    "Product 5": "PowerGuard Pro Helmet",
    "Product 6": "Endurance Training Shorts",
    "Product 7": "Victory Crew Sweatshirt",
    "Product 8": "Classic Match Trousers"
  };
...
/* Professional Slider & Animation Enhancements */
jQuery(document).ready(function($) {
    
    // Enhanced slider with fade transition
    if ($('#mainCarousel').length) {
        // Add fade class for smooth transitions
        $('#mainCarousel').addClass('carousel-fade');
        ...
        $('#mainCarousel').on('slide.bs.carousel', function (e) {
            ...
            $('.carousel-item .hero-title, .carousel-item .hero-subtitle, .carousel-item .hero-buttons, .carousel-item .hero-scroll').css({
                'opacity': '0',
                'transform': 'translateY(30px)',
                'animation': 'none'
            });
            
            // Trigger new animations after a small delay
            setTimeout(function() {
                $currentSlide.find('.hero-title').css('animation', 'fadeInUp 0.8s ease-out 0.3s forwards');
                $currentSlide.find('.hero-subtitle').css('animation', 'fadeInUp 0.8s ease-out 0.6s forwards');
                $currentSlide.find('.hero-buttons').css('animation', 'fadeInUp 0.8s ease-out 0.9s forwards');
                $currentSlide.find('.hero-scroll').css('animation', 'fadeIn 0.8s ease-out 1.2s forwards');
            }, 100);
        });
    }
    ...
    $('.grid-item, .team-member-card, .project-card, .category-card, .wc-block-product').addClass('animate-on-scroll');
    ...
    // Remove loading="lazy" from images for Edge browser compatibility
    // Edge has known issues with lazy loading causing images not to display
    ...
    // Professional preloader hide
    $(window).on('load', function() {
        $('#loadingAnimation').fadeOut(600);
    });
    ...
    // Mobile menu close on link click - ONLY for submenu items without children
    $('.mainmenu .sub-menu a').on('click', function() {
        if ($(window).width() <= 992) {
            // Check if this link's parent has children (is a parent item)
