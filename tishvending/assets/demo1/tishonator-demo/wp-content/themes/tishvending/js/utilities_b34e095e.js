...
jQuery( document ).ready(function() {
  ...
  jQuery('#primary-menu > ul > li').each(function(){
    if (jQuery(this).find('> ul').length > 0) {
      jQuery(this).addClass('menu-item dropdown');
      jQuery(this).find('> a').addClass('menu-link dropdown-toggle')
        .attr('role', 'button').attr('data-toggle', 'dropdown')
        .attr('aria-haspopup', 'true').attr('aria-expanded', 'false');

        jQuery(this).find('> ul').addClass('dropdown-menu').find('a').addClass('dropdown-item');
    } else {
      jQuery(this).addClass('menu-item').find('> a').addClass('menu-link');
    }
  });
